@Query(
    value = "UPDATE BIN_STAGES bs SET bs.BINES = (SELECT REPLACE('[' || LISTAGG('{\"rf\":\"' || e.RANGO_FINAL || '\",\"ri\":\"' || e.RANGO_INICIAL || '\",\"id\":\"' || e.INDICE || '\"}', ',') WITHIN GROUP (ORDER BY e.INDICE) || ']', '}, {\"', '},{\"') " +
            "FROM BIN_EMISOR e " +
            "WHERE e.INDICE IN (SELECT TO_NUMBER(REGEXP_SUBSTR(:INDICES, '[^,]+', 1, LEVEL)) FROM DUAL CONNECT BY REGEXP_SUBSTR(:INDICES, '[^,]+', 1, LEVEL) IS NOT NULL)) " +
